...
exports.reportePedidosEntregado = (req, res) => {
    const { fechaInicio, fechaFin } = req.query;
    camionModel.obtenerPedidosEntregados(fechaInicio, fechaFin,(err, dato) =>{
        if (err){
            console.log(err);
        }
        res.render('camion/pedidosEntregados', { pedidos : dato });
    })
};
